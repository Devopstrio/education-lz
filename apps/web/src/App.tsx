...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Anchor, Share2, Server, Repeat, AlertTriangle, GraduationCap, Microscope, BookOpen, Fingerprint, Wallet, FileText, ClipboardCheck } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">EduFoundation</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Summary" active />
            <NavItem to="/campuses" icon={<Globe size={20} />} label="Campus Enclaves" />
            <NavItem to="/research" icon={<Microscope size={20} />} label="Research Hub" />
            <NavItem to="/learning" icon={<BookOpen size={20} />} label="Learning Systems" />
            <NavItem to="/identity" icon={<Fingerprint size={20} />} label="Identity Governance" />
            <NavItem to="/compliance" icon={<ClipboardCheck size={20} />} label="Compliance Review" />
            <NavItem to="/costs" icon={<Wallet size={20} />} label="Academic FinOps" />
          </nav>
          ...
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">Campus Architect</p>
                <p className="text-[10px] text-blue-400 uppercase tracking-widest font-black">Institutional Governance</p>
              </div>
